...
function uiIconAsset(pack, index) {
  return `../assets/generated/ui-icons/${pack}-v1/${pack}-icon-${index}.webp?v=${UI_ICON_VERSION}`;
}
...
function catalogFallbackAsset(kind, id) {
  return `../assets/generated/icons/${catalogFallbackIcon(kind, id)}.webp?v=20260627-iconforge2`;
}

function catalogImgTag(kind, id, alt = "") {
  const fallback = catalogFallbackAsset(kind, id);
  return `<img src="${catalogAsset(kind, id)}" alt="${escapeHtml(alt)}" loading="eager" decoding="async" onerror="this.onerror=null;this.src='${fallback}'" />`;
}
...
const BGM_BOSS_ENTRY_DELAY_MS = 8000;
const BGM_BOSS_EXIT_GRACE_MS = 5000;
const BGM_CROSSFADE_MS = 3000;
const BGM_FADE_STEP_MS = 80;
const BGM_DUNGEON_ROTATION_STAGES = 80;
const BGM_BOSS_ROTATION_STAGES = 500;
const BGM_MIN_VARIANT_PLAY_MS = 120000;
const SFX_MIX_VOLUMES = {
  summon: 0.3,
  summonRare: 0.34,
  rebirth: 0.42,
  upgrade: 0.62,
...
function normalizeBgmEntries(kind) {
  const listKey = kind === "boss" ? "bossBgms" : "dungeonBgms";
  const fallbackKey = kind === "boss" ? "bossBgm" : "dungeonBgm";
  const rawEntries = Array.isArray(ASSETS.audio[listKey]) && ASSETS.audio[listKey].length
    ? ASSETS.audio[listKey]
    : [{ id: kind, startStage: 1, src: ASSETS.audio[fallbackKey] }];

  return rawEntries
    .map((entry, index) => {
      const normalized = typeof entry === "string" ? { src: entry } : { ...entry };
      const startStage = Math.max(1, Math.floor(Number(normalized.startStage) || 1));
      return {
        ...normalized,
        id: normalized.id || `${kind}-${index + 1}`,
        kind,
        startStage,
        trackKey: `${kind}:${normalized.id || index + 1}`
      };
    })
    .filter((entry) => Boolean(entry.src))
    .sort((a, b) => a.startStage - b.startStage);
}

function bgmStageNumber() {
  return Math.max(1, Math.floor(Number(app.enemy?.stage ?? app.save.stage) || 1));
}

function bgmTrackKind(trackKey) {
  return bgm.trackMeta?.[trackKey]?.kind || trackKey;
}

function bgmTrackRangeKey(trackKey) {
  const entry = bgm.trackMeta?.[trackKey];
  return entry ? `${entry.kind}:${entry.startStage}` : "";
}

function isCurrentBgmKind(kind) {
  return bgmTrackKind(bgm.currentTrack) === kind;
}

function bgmEntriesForStage(kind, stage) {
  const entries = bgm.catalog?.[kind] || [];
  if (!entries.length) return [];
  const unlocked = entries.filter((entry) => stage >= entry.startStage);
  if (!unlocked.length) return [entries[0]];
  const latestStart = unlocked.reduce((max, entry) => Math.max(max, entry.startStage), unlocked[0].startStage);
  return unlocked.filter((entry) => entry.startStage === latestStart);
}

function selectBgmTrack(kind) {
  const stage = bgmStageNumber();
  const entries = bgmEntriesForStage(kind, stage);
  if (!entries.length) return kind;
  const rotationStages = kind === "boss" ? BGM_BOSS_ROTATION_STAGES : BGM_DUNGEON_ROTATION_STAGES;
  const rangeStart = entries[0].startStage;
  const bucket = Math.floor(Math.max(0, stage - rangeStart) / rotationStages);
  const offset = stablePick(rangeStart * 13 + (kind === "boss" ? 5 : 3), entries.length);
  return entries[(bucket + offset) % entries.length].trackKey;
}

function keepCurrentBgmVariant(kind, selectedKey, now) {
  if (!bgm.currentTrack || bgm.currentTrack === selectedKey) return selectedKey;
  if (!isCurrentBgmKind(kind)) return selectedKey;
  if (bgmTrackRangeKey(bgm.currentTrack) !== bgmTrackRangeKey(selectedKey)) return selectedKey;
  if (now - (bgm.lastSwitchAt || 0) >= BGM_MIN_VARIANT_PLAY_MS) return selectedKey;
  return bgm.currentTrack;
}

function ensureAudioAssets() {
  if (bgm.tracks && bgm.sfx) return;
  const dungeonEntries = normalizeBgmEntries("dungeon");
  const bossEntries = normalizeBgmEntries("boss");
  const bgmEntries = [...dungeonEntries, ...bossEntries];
  bgm.catalog = {
    dungeon: dungeonEntries,
    boss: bossEntries
  };
  bgm.trackMeta = Object.fromEntries(bgmEntries.map((entry) => [entry.trackKey, entry]));
  bgm.tracks = {
    ...Object.fromEntries(bgmEntries.map((entry) => [entry.trackKey, createAudio(entry.src, { loop: true, volume: 0 })]))
  };
  bgm.sfx = {
    summon: createAudio(ASSETS.audio.summon, { volume: SFX_MIX_VOLUMES.summon }),
    summonRare: createAudio(ASSETS.audio.summonRare, { volume: SFX_MIX_VOLUMES.summonRare }),
    rebirth: createAudio(ASSETS.audio.rebirth, { volume: SFX_MIX_VOLUMES.rebirth }),
    upgrade: createAudio(ASSETS.audio.upgrade, { volume: SFX_MIX_VOLUMES.upgrade }),
    bossClear: createAudio(ASSETS.audio.bossClear, { volume: SFX_MIX_VOLUMES.bossClear }),
    attackCast: createAudio(ASSETS.audio.attackCast, { volume: SFX_MIX_VOLUMES.attackCast }),
    attackSwing: createAudio(ASSETS.audio.attackSwing, { volume: SFX_MIX_VOLUMES.attackSwing }),
    attackArrow: createAudio(ASSETS.audio.attackArrow, { volume: SFX_MIX_VOLUMES.attackArrow }),
    attackCurse: createAudio(ASSETS.audio.attackCurse, { volume: SFX_MIX_VOLUMES.attackCurse }),
    hitLight: createAudio(ASSETS.audio.hitLight, { volume: SFX_MIX_VOLUMES.hitLight }),
    hitHeavy: createAudio(ASSETS.audio.hitHeavy, { volume: SFX_MIX_VOLUMES.hitHeavy }),
    criticalHit: createAudio(ASSETS.audio.criticalHit, { volume: SFX_MIX_VOLUMES.criticalHit }),
    enemyDown: createAudio(ASSETS.audio.enemyDown, { volume: SFX_MIX_VOLUMES.enemyDown })
  };
}

function clearBgmSyncTimer() {
  if (!bgm.syncTimer) return;
  window.clearTimeout(bgm.syncTimer);
  bgm.syncTimer = null;
}

function scheduleBgmSync(delayMs) {
  clearBgmSyncTimer();
  if (!app.settings.bgmEnabled) return;
  bgm.syncTimer = window.setTimeout(() => {
    bgm.syncTimer = null;
    void syncBgm();
  }, Math.max(0, delayMs));
}
...
function stopBgmFade() {
  if (!bgm.fadeTimer) return;
  window.clearInterval(bgm.fadeTimer);
  bgm.fadeTimer = null;
}

function markBgmEncounter(enemy) {
  const now = performance.now();
  clearBgmSyncTimer();
  if (enemy?.boss) {
    bgm.bossStartedAt = now;
    scheduleBgmSync(BGM_BOSS_ENTRY_DELAY_MS + 50);
    return;
  }

  bgm.bossStartedAt = 0;
  if (isCurrentBgmKind("boss")) {
    const hasPendingReturn = bgm.returnDungeonAt && now < bgm.returnDungeonAt;
    if (!hasPendingReturn) bgm.returnDungeonAt = now + BGM_BOSS_EXIT_GRACE_MS;
    scheduleBgmSync(bgm.returnDungeonAt - now + 50);
  } else {
    bgm.returnDungeonAt = 0;
  }
}

function bgmTrackVolume(key) {
  const base = bgmTrackKind(key) === "boss" ? 0.62 : 0.58;
  return base * normalizeVolumeSetting(app.settings.bgmVolume);
}

function desiredBgmTrack() {
  const now = performance.now();
  const dungeonTrack = keepCurrentBgmVariant("dungeon", selectBgmTrack("dungeon"), now);
  const bossTrack = keepCurrentBgmVariant("boss", selectBgmTrack("boss"), now);
  if (app.enemy?.boss) {
    const bossElapsed = bgm.bossStartedAt ? now - bgm.bossStartedAt : 0;
    if (bossElapsed >= BGM_BOSS_ENTRY_DELAY_MS) return bossTrack;
    return isCurrentBgmKind("boss") ? bgm.currentTrack : dungeonTrack;
  }

  if (isCurrentBgmKind("boss") && bgm.returnDungeonAt && now < bgm.returnDungeonAt) {
    return bgm.currentTrack;
  }
  return dungeonTrack;
}

async function playBgmTrack(track) {
...
async function fadeBgmTo(activeKey) {
  const tracks = bgm.tracks;
  const activeTrack = tracks[activeKey];
  if (!activeTrack) return;
  const previousKey = bgm.currentTrack;
  const previousTrack = previousKey && previousKey !== activeKey ? tracks[previousKey] : null;
  const targetVolume = bgmTrackVolume(activeKey);

  stopBgmFade();
  Object.entries(tracks).forEach(([key, track]) => {
    if (key !== activeKey && key !== previousKey) {
      track.pause();
      track.volume = 0;
    }
  });

  if (previousKey === activeKey && !activeTrack.paused) {
  ...
  const canPlay = await playBgmTrack(activeTrack);
  if (!canPlay) return;

  bgm.currentTrack = activeKey;
  bgm.lastSwitchAt = performance.now();
  if (bgmTrackKind(activeKey) === "dungeon") bgm.returnDungeonAt = 0;
  if (!previousTrack || previousTrack.paused || BGM_CROSSFADE_MS <= 0) {
    activeTrack.volume = targetVolume;
    Object.entries(tracks).forEach(([key, track]) => {
      if (key !== activeKey) {
        track.pause();
  ...
  const previousStartVolume = previousTrack.volume;
  activeTrack.volume = Math.min(activeTrack.volume, targetVolume);
  bgm.fadeTimer = window.setInterval(() => {
    const progress = Math.min(1, (performance.now() - fadeStartedAt) / BGM_CROSSFADE_MS);
    activeTrack.volume = targetVolume * progress;
    previousTrack.volume = previousStartVolume * (1 - progress);
